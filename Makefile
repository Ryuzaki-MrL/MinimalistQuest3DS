...
#---------------------------------------------------------------------------------
APP_TITLE 		:=	Minimalist Quest
APP_DESCRIPTION	:=	Save your friends and escape the dungeon
...
TARGET		    :=	MinimalistQuest
OUTDIR			:=	out
BUILD		    :=	build
SOURCES		    :=	source source/ECS source/ECS/Component source/ECS/Entity source/State source/util
DATA		    :=	data
INCLUDES	    :=	$(SOURCES)
GRAPHICS		:=	assets/gfx assets/gfx/fontmain assets/gfx/arial9 assets/gfx/tiles assets/gfx/sprites
ROMFS		    :=	romfs
GFXBUILD		:=	$(ROMFS)
...
all:
	...
	@makerom -f cia -o $(OUTPUT).cia -target t -exefslogo -elf "$(OUTPUT).elf" -rsf "$(RSF_PATH)" -ver "$$(($(VERSION_MAJOR)*1024+$(VERSION_MINOR)*16+$(VERSION_MICRO)))" -banner "$(BUILD)/banner.bnr" -icon "$(BUILD)/icon.icn" -DAPP_TITLE="$(APP_TITLE)" -DAPP_PRODUCT_CODE="$(PRODUCT_CODE)" -DAPP_UNIQUE_ID="$(UNIQUE_ID)" -DAPP_ROMFS="$(ROMFS)" -logo "$(LOGO)"
#---------------------------------------------------------------------------------
clean:
	@echo clean ...
	@rm -fr $(BUILD) $(OUTDIR)
